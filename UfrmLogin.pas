...
begin
  ...
  sql := ' select user_kode,user_nama' +
         ' from tuser  ' +
         ' where ' +
  ...
  begin
    try
      ...
      begin
        ...
        frmMenu.LoadMenu(Fields[0].AsString);
        frmMenu.ApplyNavBarItems(frmMenu.dxNavBar2);
        frmMenu.ApplyNavBarGroups(frmMenu.dxNavBar2);
...
var
  ...
  tsql :TmyQuery;
begin
  s := 'select cbg_kode,cbg_nama,cbg_rekeningops from tcabang where cbg_aktif=1';
  tsql := xOpenQuery(S, frmMenu.conn);
  with tsql do
  begin
    try
      ...
      begin
        ...
        frmMenu.RKCABANG := Fields[2].AsString;
