object frmBrowseEstimasiSales: TfrmBrowseEstimasiSales
  Left = 277
  Top = 38
  Width = 1024
  Height = 644
  Caption = 'Laporan Estimasi Salesman'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object AdvPanel1: TAdvPanel
    Left = 0
    Top = 41
    Width = 1008
    Height = 64
    Align = alTop
    BevelInner = bvRaised
    Color = clWhite
    TabOrder = 0
    UseDockManager = True
    AnchorHint = False
    AutoSize.Enabled = False
    AutoSize.Height = True
    AutoSize.Width = True
    AutoHideChildren = True
    BackgroundPosition = bpTopLeft
    BorderColor = clBlack
    BorderShadow = False
    Buffered = True
    CanMove = False
    CanSize = False
    Caption.ButtonPosition = cbpRight
    Caption.Color = clHighlight
    Caption.ColorTo = clNone
    Caption.CloseColor = clBtnFace
    Caption.CloseButton = False
    Caption.CloseButtonColor = clWhite
    Caption.Flat = False
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clHighlightText
    Caption.Font.Height = -11
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.Height = 20
    Caption.Indent = 0
    Caption.MinMaxButton = False
    Caption.MinMaxButtonColor = clWhite
    Caption.ShadeLight = 200
    Caption.ShadeGrain = 32
    Caption.ShadeType = stNormal
    Caption.Shape = csRectangle
    Caption.TopIndent = 0
    Caption.Visible = False
    Collaps = False
    CollapsColor = clGray
    CollapsDelay = 20
    CollapsSteps = 0
    ColorTo = clNone
    FixedTop = False
    FixedLeft = False
    FixedHeight = False
    FixedWidth = False
    FreeOnClose = False
    Hover = False
    HoverColor = clNone
    HoverFontColor = clNone
    Indent = 0
    LineSpacing = 0
    Position.Save = False
    Position.Location = clRegistry
    ShadowColor = clGray
    ShadowOffset = 2
    ShowMoveCursor = False
    TextVAlign = tvaTop
    TopIndent = 0
    URLColor = clBlue
    FullHeight = 0
    object Label1: TLabel
      Left = 19
      Top = 10
      Width = 36
      Height = 13
      Caption = 'Periode'
    end
    object Label4: TLabel
      Left = 17
      Top = 33
      Width = 30
      Height = 13
      Caption = 'Tahun'
    end
    object cbbBulan: TAdvComboBox
      Left = 81
      Top = 6
      Width = 145
      Height = 21
      Color = clWindow
      AutoFocus = False
      ButtonWidth = 19
      Flat = True
      FlatLineColor = clBlack
      FlatParentColor = True
      Etched = False
      FocusBorder = False
      DropWidth = 0
      Enabled = True
      ItemHeight = 13
      Items.Strings = (
        'Januari'
        'Februari'
        'Maret'
        'April'
        'Mei'
        'Juni'
        'Juli'
        'Agustus'
        'September'
        'Oktober'
        'November'
        'Desember')
      LabelPosition = lpLeftTop
      LabelMargin = 4
      LabelTransparent = False
      LabelAlwaysEnabled = False
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'MS Sans Serif'
      LabelFont.Style = []
      TabOrder = 0
    end
    object edtTahun: TComboBox
      Left = 81
      Top = 29
      Width = 145
      Height = 21
      ItemHeight = 13
      TabOrder = 1
      Text = '2017'
      Items.Strings = (
        '2016'
        '2017'
        '2018'
        '2019'
        '2020'
        '2021')
    end
    object btnRefresh: TcxButton
      Left = 234
      Top = 21
      Width = 87
      Height = 27
      Caption = '&Load '
      TabOrder = 2
      OnClick = btnRefreshClick
      Glyph.Data = {
        
        0000009C0000FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.SkinName = 'LondonLiquidSky'
    end
    object cxButton1: TcxButton
      Left = 330
      Top = 21
      Width = 143
      Height = 27
      Caption = 'Ambil data Kunjungan'
      TabOrder = 3
      OnClick = cxButton1Click
      Glyph.Data = {
        
        0000009C0000FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.SkinName = 'LondonLiquidSky'
    end
  end
  object AdvPanel3: TAdvPanel
    Left = 0
    Top = 570
    Width = 1008
    Height = 35
    Align = alBottom
    BevelInner = bvRaised
    Color = clWhite
    TabOrder = 1
    UseDockManager = True
    AnchorHint = False
    AutoSize.Enabled = False
    AutoSize.Height = True
    AutoSize.Width = True
    AutoHideChildren = True
    BackgroundPosition = bpTopLeft
    BorderColor = clBlack
    BorderShadow = False
    Buffered = True
    CanMove = False
    CanSize = False
    Caption.ButtonPosition = cbpRight
    Caption.Color = clHighlight
    Caption.ColorTo = clNone
    Caption.CloseColor = clBtnFace
    Caption.CloseButton = False
    Caption.CloseButtonColor = clWhite
    Caption.Flat = False
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clHighlightText
    Caption.Font.Height = -11
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.Height = 20
    Caption.Indent = 0
    Caption.MinMaxButton = False
    Caption.MinMaxButtonColor = clWhite
    Caption.ShadeLight = 200
    Caption.ShadeGrain = 32
    Caption.ShadeType = stNormal
    Caption.Shape = csRectangle
    Caption.TopIndent = 0
    Caption.Visible = False
    Collaps = False
    CollapsColor = clGray
    CollapsDelay = 20
    CollapsSteps = 0
    ColorTo = clNone
    FixedTop = False
    FixedLeft = False
    FixedHeight = False
    FixedWidth = False
    FreeOnClose = False
    Hover = False
    HoverColor = clNone
    HoverFontColor = clNone
    Indent = 0
    LineSpacing = 0
    Position.Save = False
    Position.Location = clRegistry
    ShadowColor = clGray
    ShadowOffset = 2
    ShowMoveCursor = False
    TextVAlign = tvaTop
    TopIndent = 0
    URLColor = clBlue
    FullHeight = 0
    object cxButton8: TcxButton
      Left = 919
      Top = 2
      Width = 87
      Height = 31
      Caption = '&Tutup'
      TabOrder = 0
      OnClick = cxButton8Click
      Align = alRight
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF006D9CD4896A9AD2FB6697CFEE}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.SkinName = 'LondonLiquidSky'
    end
    object cxButton7: TcxButton
      Left = 2
      Top = 2
      Width = 87
      Height = 31
      Caption = '&Export'
      TabOrder = 1
      OnClick = cxButton7Click
      Align = alLeft
      Glyph.Data = {
        
        8400E7AD8400DEAD7B00DEA57B00E7B59400FF00FF00FF00FF00}
      LookAndFeel.Kind = lfStandard
      LookAndFeel.SkinName = 'LondonLiquidSky'
    end
  end
  object AdvPanel2: TAdvPanel
    Left = 0
    Top = 0
    Width = 1008
    Height = 41
    Align = alTop
    BevelInner = bvRaised
    Color = clWhite
    TabOrder = 2
    UseDockManager = True
    AnchorHint = False
    AutoSize.Enabled = False
    AutoSize.Height = True
    AutoSize.Width = True
    AutoHideChildren = True
    BackgroundPosition = bpTopLeft
    BorderColor = clBlack
    BorderShadow = False
    Buffered = True
    CanMove = False
    CanSize = False
    Caption.ButtonPosition = cbpRight
    Caption.Color = clHighlight
    Caption.ColorTo = clNone
    Caption.CloseColor = clBtnFace
    Caption.CloseButton = False
    Caption.CloseButtonColor = clWhite
    Caption.Flat = False
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clHighlightText
    Caption.Font.Height = -11
    Caption.Font.Name = 'MS Sans Serif'
    Caption.Font.Style = []
    Caption.Height = 20
    Caption.Indent = 0
    Caption.MinMaxButton = False
    Caption.MinMaxButtonColor = clWhite
    Caption.ShadeLight = 200
    Caption.ShadeGrain = 32
    Caption.ShadeType = stNormal
    Caption.Shape = csRectangle
    Caption.TopIndent = 0
    Caption.Visible = False
    Collaps = False
    CollapsColor = clGray
    CollapsDelay = 20
    CollapsSteps = 0
    ColorTo = clNone
    FixedTop = False
    FixedLeft = False
    FixedHeight = False
    FixedWidth = False
    FreeOnClose = False
    Hover = False
    HoverColor = clNone
    HoverFontColor = clNone
    Indent = 0
    LineSpacing = 0
    Position.Save = False
    Position.Location = clRegistry
    ShadowColor = clGray
    ShadowOffset = 2
    ShowMoveCursor = False
    TextVAlign = tvaTop
    TopIndent = 0
    URLColor = clBlue
    FullHeight = 0
    object lbljudul: TLabel
      Left = 15
      Top = 11
      Width = 222
      Height = 19
      Caption = 'Laporan Estimasi Salesman'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object AdvPanel4: TAdvPanel
    Left = 0
    Top = 105
    Width = 1008
    Height = 465
    Align = alClient
    BevelInner = bvRaised
    Color = clWhite
    TabOrder = 3
    UseDockManager = True
    AnchorHint = False
    AutoSize.Enabled = False
    AutoSize.Height = True
    AutoSize.Width = True
    AutoHideChildren = True
    BackgroundPosition = bpTopLeft
    BorderColor = clBlack
    BorderShadow = False
    Buffered = True
    CanMove = False
    CanSize = False
    Caption.ButtonPosition = cbpRight
    Caption.Color = clHighlight
    Caption.ColorTo = clNone
    Caption.CloseColor = clBtnFace
    Caption.CloseButton = False
    Caption.CloseButtonColor = clWhite
    Caption.Flat = False
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clHighlightText
    Caption.Font.Height = -11
    Caption.Font.Name = 'MS Sans Serif'
    Caption.Font.Style = []
    Caption.Height = 20
    Caption.Indent = 0
    Caption.MinMaxButton = False
    Caption.MinMaxButtonColor = clWhite
    Caption.ShadeLight = 200
    Caption.ShadeGrain = 32
    Caption.ShadeType = stNormal
    Caption.Shape = csRectangle
    Caption.TopIndent = 0
    Caption.Visible = False
    Collaps = False
    CollapsColor = clGray
    CollapsDelay = 20
    CollapsSteps = 0
    ColorTo = clNone
    FixedTop = False
    FixedLeft = False
    FixedHeight = False
    FixedWidth = False
    FreeOnClose = False
    Hover = False
    HoverColor = clNone
    HoverFontColor = clNone
    Indent = 0
    LineSpacing = 0
    Position.Save = False
    Position.Location = clRegistry
    ShadowColor = clGray
    ShadowOffset = 2
    ShowMoveCursor = False
    TextVAlign = tvaTop
    TopIndent = 0
    URLColor = clBlue
    FullHeight = 0
    object cxGrid: TcxGrid
      Left = 2
      Top = 2
      Width = 1004
      Height = 461
      Align = alClient
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      object cxGrdMain: TcxGridDBTableView
        OnKeyDown = cxGrdMainKeyDown
        NavigatorButtons.ConfirmDelete = False
        DataController.Filter.Options = [fcoCaseInsensitive]
        DataController.Summary.DefaultGroupSummaryItems = <
          item
            Format = ',0;(,0)'
            Kind = skSum
            Position = spFooter
            Column = clTarget
          end
          item
            Format = ',0;(,0)'
            Column = clTarget
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Position = spFooter
            Column = clEstimasi
          end
          item
            Format = ',0;(,0)'
            Column = clEstimasi
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Position = spFooter
            Column = clRealisasi
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Position = spFooter
            Column = clTargetInkaso
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Position = spFooter
            Column = clEstimasiInkaso
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Position = spFooter
            Column = clRealisasiInkaso
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = CLpf
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Position = spFooter
            Column = CLpf
          end>
        DataController.Summary.FooterSummaryItems = <
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = clEstimasi
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = clTarget
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = clRealisasi
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = clTargetInkaso
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = clRealisasiInkaso
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = clEstimasiInkaso
          end
          item
            OnGetText = cxGrdMainTcxGridDBDataControllerTcxDataSummaryFooterSummaryItems6GetText
            Column = clRatio
          end
          item
            OnGetText = cxGrdMainTcxGridDBDataControllerTcxDataSummaryFooterSummaryItems7GetText
            Column = clRatio2
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = CLpf
          end
          item
            Format = ',0;(,0)'
            Kind = skSum
            Column = clavgsales
          end>
        DataController.Summary.SummaryGroups = <>
        FilterRow.ApplyChanges = fracImmediately
        OptionsBehavior.GoToNextCellOnEnter = True
        OptionsBehavior.FocusCellOnCycle = True
        OptionsCustomize.ColumnHiding = True
        OptionsData.Appending = True
        OptionsData.CancelOnExit = False
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsView.Footer = True
        OptionsView.GroupFooters = gfAlwaysVisible
        OptionsView.HeaderHeight = 23
        object clNo: TcxGridDBColumn
          DataBinding.FieldName = 'No'
          PropertiesClassName = 'TcxSpinEditProperties'
          OnGetDisplayText = clNoGetDisplayText
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Options.Focusing = False
          Width = 36
        end
        object clKodeSalesman: TcxGridDBColumn
          DataBinding.FieldName = 'kode_salesman'
          Visible = False
        end
        object clSalesman: TcxGridDBColumn
          Caption = 'Salesman'
          DataBinding.FieldName = 'salesman'
          GroupSummaryAlignment = taRightJustify
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Width = 83
        end
        object clMarketing: TcxGridDBColumn
          Caption = 'Marketing'
          DataBinding.FieldName = 'marketing'
          PropertiesClassName = 'TcxTextEditProperties'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 104
        end
        object clCustomer: TcxGridDBColumn
          Caption = 'Kode Customer'
          DataBinding.FieldName = 'Kode'
          PropertiesClassName = 'TcxButtonEditProperties'
          Properties.Buttons = <
            item
              Default = True
              Kind = bkEllipsis
            end>
          Properties.OnButtonClick = clCustomerPropertiesButtonClick
          Visible = False
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Width = 97
        end
        object clNamaCustomer: TcxGridDBColumn
          Caption = 'Nama Customer'
          DataBinding.FieldName = 'Nama'
          PropertiesClassName = 'TcxTextEditProperties'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Options.Focusing = False
          Width = 254
        end
        object CLpf: TcxGridDBColumn
          Caption = 'PF'
          DataBinding.FieldName = 'pf'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;(,0.)'
          Properties.ReadOnly = True
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 107
        end
        object clTarget: TcxGridDBColumn
          Caption = 'Target Sales'
          DataBinding.FieldName = 'targetsales'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;(,0.)'
          FooterAlignmentHorz = taRightJustify
          GroupSummaryAlignment = taRightJustify
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Width = 102
        end
        object clavgsales: TcxGridDBColumn
          Caption = 'AvgSales'
          DataBinding.FieldName = 'avgsales'
          GroupSummaryAlignment = taCenter
          HeaderAlignmentHorz = taCenter
          Width = 140
        end
        object clEstimasi: TcxGridDBColumn
          Caption = 'Estimasi Sales'
          DataBinding.FieldName = 'estimasisales'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;(,0.)'
          FooterAlignmentHorz = taRightJustify
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Width = 99
        end
        object clRealisasi: TcxGridDBColumn
          Caption = 'Realisasi Sales'
          DataBinding.FieldName = 'realisasisales'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;(,0.)'
          FooterAlignmentHorz = taRightJustify
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Options.Focusing = False
          Width = 103
        end
        object clRatio: TcxGridDBColumn
          Caption = 'Ratio'
          DataBinding.FieldName = 'ratiosales'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Options.Focusing = False
          Width = 83
        end
        object clKendala: TcxGridDBColumn
          Caption = 'Kendala'
          Visible = False
          Styles.Content = cxStyle1
          Width = 105
        end
        object clAction: TcxGridDBColumn
          Caption = 'Action'
          Visible = False
          Styles.Content = cxStyle1
          Width = 110
        end
        object clSolusi: TcxGridDBColumn
          Caption = 'Solusi'
          Visible = False
          Styles.Content = cxStyle1
          Width = 130
        end
        object clTargetInkaso: TcxGridDBColumn
          Caption = 'Target Inkaso'
          DataBinding.FieldName = 'targetinkaso'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;(,0.)'
          FooterAlignmentHorz = taRightJustify
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Options.Focusing = False
          Width = 100
        end
        object clEstimasiInkaso: TcxGridDBColumn
          Caption = 'Estimasi Inkaso'
          DataBinding.FieldName = 'estimasiinkaso'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;(,0.)'
          FooterAlignmentHorz = taRightJustify
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Width = 99
        end
        object clRealisasiInkaso: TcxGridDBColumn
          Caption = 'Realisasi Inkaso'
          DataBinding.FieldName = 'realisasiinkaso'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;(,0.)'
          FooterAlignmentHorz = taRightJustify
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Options.Focusing = False
          Width = 92
        end
        object clRatio2: TcxGridDBColumn
          Caption = 'Ratio'
          DataBinding.FieldName = 'ratioinkaso'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Options.Editing = False
          Options.Focusing = False
          Width = 45
        end
        object cltglbayar: TcxGridDBColumn
          Caption = 'Tanggal Bayar'
          Visible = False
          Styles.Content = cxStyle1
          Width = 98
        end
        object clsolusi2: TcxGridDBColumn
          Caption = 'Solusi'
          Visible = False
          Styles.Content = cxStyle1
          Width = 97
        end
        object clKunjunganSales: TcxGridDBColumn
          Caption = 'Salesman'
          DataBinding.FieldName = 'ksalesman'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;($,0.)'
          Properties.ReadOnly = True
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          HeaderGlyphAlignmentHorz = taCenter
          Width = 75
        end
        object clKunjunganMarketing: TcxGridDBColumn
          Caption = 'Marketing'
          DataBinding.FieldName = 'kMarketing'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Properties.DisplayFormat = ',0.;(,0.)'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
        end
      end
      object cxGrdDetail: TcxGridDBTableView
        NavigatorButtons.ConfirmDelete = False
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsData.CancelOnExit = False
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.GroupByBox = False
      end
      object lvMaster: TcxGridLevel
        Caption = 'Master Data'
        GridView = cxGrdMain
      end
    end
  end
  object savedlg: TSaveDialog
    Left = 256
    Top = 80
  end
  object MyConnection1: TMyConnection
    Database = 'bsm'
    Username = 'bsm'
    Password = 'Zainal_12345'
    Server = '194.59.165.164'
    LoginPrompt = False
    Left = 560
    Top = 185
  end
  object MyQuery1: TMyQuery
    Connection = MyConnection1
    Left = 584
    Top = 241
  end
  object cxStyleRepository1: TcxStyleRepository
    Left = 440
    Top = 241
    PixelsPerInch = 96
    object cxStyle1: TcxStyle
      AssignedValues = [svColor]
      Color = clYellow
    end
  end
end
