...
type
  TfrmKomisisalesman = class(TForm)
    ...
    sqlqry1: TMyQuery;
    sqlqry2: TMyQuery;
    clRealisasiOther: TcxGridDBBandedColumn;
    clTargetOther: TcxGridDBBandedColumn;
   ...
   function getperseninkaso4(apersen:double) : double;
    ...
    procedure cxGrid1DBBandedTableView1TcxGridDBDataControllerTcxDataSummaryFooterSummaryItems4GetText(
      Sender: TcxDataSummaryItem; const AValue: Variant;
      AIsFooter: Boolean; var AText: String);
    procedure cxGrid1DBBandedTableView1TcxGridDBDataControllerTcxDataSummaryFooterSummaryItems14GetText(
      Sender: TcxDataSummaryItem; const AValue: Variant;
      AIsFooter: Boolean; var AText: String);

   function cekmasakerja(asales: string): Boolean;
  private
    flagedit : Boolean;
    fid : integer;
    fnomorjual : string ;
...
begin
  ...
  begin
    ...
    akhir3 := EndOfTheMonth(StrToDate(IntToStr(cbbBulan.itemindex )+'/01/'+inttostr(StrToInt(edtTahun.text)-1)));
    akhir4 := EndOfTheMonth(StrToDate(IntToStr(cbbBulan.itemindex+1 )+'/01/'+inttostr(StrToInt(edtTahun.text)-1)));
  end
  else
  begin
    akhir := EndOfTheMonth(StrToDate('12/01/'+inttostr(StrToInt(edtTahun.text)-1)));
    akhir2 := EndOfTheMonth(StrToDate(IntToStr(cbbBulan.itemindex+1 )+'/01/'+edttahun.Text));
    akhir3 := EndOfTheMonth(StrToDate('12/01/'+inttostr(StrToInt(edtTahun.text)-2)));
    akhir4 := EndOfTheMonth(StrToDate(IntToStr(cbbBulan.itemindex+1 )+'/01/'+inttostr(StrToInt(edtTahun.text)-1)));
  end;
  //s:='SET @tg1='+quotd(StrToDate(IntToStr(cbbBulan.itemindex+1 )+'/01/'+edttahun.Text));
  // EnsureConnected(frmMenu.conn);
  //  ExecSQLDirect(frmMenu.conn, s);
  //s:='SET @tg2='+quotd(akhir2);

  s:= 'set @tg1 ='+quotd(StrToDate(IntToStr(cbbBulan.itemindex+1 )+'/01/'+edttahun.Text))+';';
  s:= s +'set @tg2 ='+quotd(akhir2)+';';
  EnsureConnected(frmMenu.conn);
  ExecSQLDirect(frmMenu.conn, s);
...
+ ' (riil3+IFNULL(girolalu3,0)-kontrak3-biaya_promosi3 - fee_marketing3) pengali3,'
+ ' (riil4+IFNULL(girolalu4,0)-kontrak4-biaya_promosi4 - fee_marketing4) pengali4,'
+ ' riil komisi_jual,riil2 persen,riil3 nilai,riil4 total,riil2 persen2,riil3 nilai2,riil2 persen3,riil3 nilai3,riil2 persen4,riil3 nilai4,cast(0 as decimal) komisipf, cast(0 as decimal) komisiothers, '
...
+ ' IFNULL(fee_marketing,0) + IFNULL(fee_marketing2,0) + IFNULL(fee_marketing3,0) + IFNULL(fee_marketing4,0) fee_marketing '
+ ' FROM ('
...
+ ' SUM(fee_marketing) fee_marketing, SUM(fee_marketing2) fee_marketing2, '
+ ' SUM(fee_marketing3) fee_marketing3, SUM(fee_marketing4) fee_marketing4, '
+ ' ('
...
+ ' (SELECT SUM((bycd_bayar/fp_amount)*((Fp_biayapr*((fp_amount-fp_taxamount- IFNULL(('
...
+ '     SELECT SUM('
+ '         (Fp_biayapr2 * '
+ '         (fp_amount - fp_taxamount - IFNULL('
+ '             (SELECT SUM(retj_amount)'
+ '              FROM tretj_hdr'
+ '              WHERE retj_fp_nomor = fp_nomor),0'
+ '         )))/100 + fp_biayarp2'
+ '     )'
+ '     FROM tbayarcus_dtl'
+ '     INNER JOIN tfp_hdr ON fp_nomor = bycd_fp_nomor'
...
+ '     AND DATEDIFF(byc_tanggal,fp_tanggal) <= 40'
+ ' ) Fee_Marketing,'
+ ' ('
+ '     SELECT SUM('
+ '         (Fp_biayapr2 * '
...
//          end;
          ...
          // === HITUNGAN LAMA
          {
          if ds3.fieldbyname('target_jual').AsFloat < 100000000 then
             apotong := 0.5
           else
           apotong := 1;
...
//            if (ds32.FieldByName('komisi_jual').asfloat > 0) and (ds3.FieldByName('ratiopf').asfloat > 20) then
        ...
        ds32.FieldByName('komisiothers').asfloat := 0;
        if (ds3.fieldbyname('PresentaseOther').AsFloat >= 120) and (ds3.fieldbyname('Realisasi_Jual_Other').AsFloat >= 100000000) then
          ds32.FieldByName('komisiothers').asfloat := 400000;

        ds32.FieldByName('total').asfloat :=ds32.FieldByName('komisipf').asfloat+ds32.FieldByName('komisiothers').asfloat+ds32.FieldByName('komisi_jual').asfloat+ds32.FieldByName('nilai').asfloat+ds32.FieldByName('nilai2').asfloat+ds32.FieldByName('nilai3').asfloat+ds32.FieldByName('nilai4').asfloat;
...
function TfrmKomisisalesman.cekmasakerja(asales: string): Boolean;
var
  i: integer;
  s: string;
  tsql: TmyQuery;
begin
  ...
  try
    ...
    begin
     if not eof then
     begin
       if FieldByName('masuk').AsInteger > 3 then result := True;
     end;
    end;
   finally
     tsql.Free;
...
procedure TfrmKomisisalesman.cxGrid1DBBandedTableView1TcxGridDBDataControllerTcxDataSummaryFooterSummaryItems14GetText(
  Sender: TcxDataSummaryItem; const AValue: Variant; AIsFooter: Boolean;
  var AText: String);
  var
    capaibulanini :double;
