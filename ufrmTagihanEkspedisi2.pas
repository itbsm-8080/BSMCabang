...
type
  TfrmTagihanEkspedisi2 = class(TForm)
    AdvPanel1: TAdvPanel;
    AdvPanel3: TAdvPanel;
    cxButton8: TcxButton;
    AdvPanel2: TAdvPanel;
...
procedure TfrmTagihanEkspedisi2.FormCreate(Sender: TObject);
begin
  TcxDBGridHelper(cxGrdMain).LoadFromCDS(CDS, False, False);
end;

procedure TfrmTagihanEkspedisi2.refreshdata;
begin
  FID:='';
  dttanggal.Date := Date;
  startdate.Date := Date;
...
procedure TfrmTagihanEkspedisi2.initgrid;
begin
  CDS.EmptyDataSet;
  CDS.Append;
  CDS.Post;
...
procedure TfrmTagihanEkspedisi2.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_F8 then
  begin
...
procedure TfrmTagihanEkspedisi2.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if Key = #13 then
...
procedure TfrmTagihanEkspedisi2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action := caFree;
   Release;
end;

procedure TfrmTagihanEkspedisi2.cxButton8Click(Sender: TObject);
begin
  Release;
end;

procedure TfrmTagihanEkspedisi2.FormShow(Sender: TObject);
begin
  refreshdata;
  bacafile2;
...
function TfrmTagihanEkspedisi2.GetCDS: TClientDataSet;
begin
  ...
  begin
    ...
    zAddField(FCDS, 'stenomor', ftString, False,25);
...
procedure TfrmTagihanEkspedisi2.clNoGetDisplayText(Sender: TcxCustomGridTableItem;
...
procedure TfrmTagihanEkspedisi2.cxGrdMainKeyDown(Sender: TObject; var Key: Word;
    Shift: TShiftState);
begin
...
procedure TfrmTagihanEkspedisi2.bacafile2;
var
s:string;
tsql:tmyquery;
...
procedure TfrmTagihanEkspedisi2.insertketampungan(anomor:string);
var
  s:string;
  tsql : TmyQuery;
  a,i,x:integer;
...
procedure TfrmTagihanEkspedisi2.cxButton7Click(Sender: TObject);
begin
...
procedure TfrmTagihanEkspedisi2.btnRefreshClick(Sender: TObject);
var
    s:string;
    tsql:TmyQuery;
    i:Integer;
begin
     ...
     s := 'SELECT fp_nomor, fp_tanggal, fp_cus_kode, cus_nama, (fp_amount-fp_taxamount)fp_amount, fp_taxamount, ste_nomor '
        + ' FROM tserahterimaekspedisi_hdr a '
        + ' INNER JOIN tserahterimaekspedisi_dtl ON  ste_nomor = sted_ste_nomor '
        + ' INNER JOIN tfp_hdr ON sted_fp_nomor = FP_nomor '
        + ' INNER JOIN tcustomer b ON b.Cus_kode = FP_cus_kode '
     ...
     begin
        try
          ...
          begin
            ...
            CDS.FieldByName('stenomor').AsString   := Fields[6].AsString;
...
function TfrmTagihanEkspedisi2.getUoc(akode:string):Integer;
var
  s:string;
  tsql:TmyQuery;
begin
...
procedure TfrmTagihanEkspedisi2.cxButton2Click(Sender: TObject);
begin
   try
...
procedure TfrmTagihanEkspedisi2.simpandata;
var
  s:string;
  aisecer,i:integer;
  tt:TStrings;
begin
  ...
  begin
   if CDS.FieldByName('biaya').Asfloat >  0 then
     begin
       if CDS.FieldByName('Nomor').AsString = '' then
          s := 'INSERT INTO ttagihanekspedisi_dtl (tehd_teh_nomor, tehd_fp_nomor, tehd_nilai, tehd_biaya, tehd_cus_nama, tehd_ste_nomor, tehd_tanggal) VALUES ('
          + Quot(edtNomor.Text) +','
          ...
          + Quot(CDS.FieldByName('stenomor').AsString) + ','
...
function TfrmTagihanEkspedisi2.getmaxkode:string;
var
  s:string;
begin
...
procedure TfrmTagihanEkspedisi2.clbiayaPropertiesEditValueChanged(Sender: TObject);
var
  i: Integer;
  lVal: Double;
...
procedure TfrmTagihanEkspedisi2.cxButton1Click(Sender: TObject);
begin
   try
//      If not cekdata then exit;
...
function TfrmTagihanEkspedisi2.cekdata:Boolean;
var
  i:integer;
begin
  result:=true;
...
procedure TfrmTagihanEkspedisi2.HapusRecord1Click(Sender: TObject);
begin
 If CDS.Eof then exit;
  CDS.Delete;
  If CDS.Eof then initgrid;
...
procedure TfrmTagihanEkspedisi2.teslip(anomor : string );
var
  s: string ;
  ftsreport : TTSReport;
begin
