...
(************************************************************
  ...
  ---------------------
  This class is made based on Epson LQ-300 printer and IBM
...
uses
  Windows, SysUtils, Classes, WinSpool;
...
type
  ...
  public
    ...
    procedure WriteList(const TextList: TStrings; IsFormatted: boolean = false);
    procedure WriteColumn(Text: string; Width: integer; Alignment: TAlignment);

    property InitPrinterRequired: boolean read fInitPrinter write fInitPrinter;
  ...
  published
    ...
    // page configuration
    property EjectOnFinish: boolean read fEjectPaper write fEjectPaper;
...
procedure TRAWPrinter.WriteList(const TextList: TStrings; IsFormatted: boolean = false);
var
  i: integer;
begin
  // write a TStrings content at once
  for i := 0 to TextList.Count-1 do 
    if IsFormatted then
      WriteLn(DecodeFormattedText(TextList.Strings[i]))
    else
      WriteLn(TextList.Strings[i]);
end;
...
procedure Register;
begin
  RegisterComponents('BeeSoft', [TRAWPrinter]);
end;
