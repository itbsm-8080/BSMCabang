...
type
  TfrmSetingFeeMarketing = class(TForm)
    PANEL: TAdvPanel;
    Label2: TLabel;
    edtNomor: TAdvEdit;
    AdvPanel3: TAdvPanel;
...
procedure TfrmSetingFeeMarketing.refreshdata;
begin
  FID:='';
...
procedure TfrmSetingFeeMarketing.initgrid;
begin
  CDS.EmptyDataSet;
  CDS.Append;
  CDS.Post;
...
procedure TfrmSetingFeeMarketing.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if Key = #13 then
...
procedure TfrmSetingFeeMarketing.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action := caFree;
   Release;
end;

function TfrmSetingFeeMarketing.getmaxkode:string;
var
  s:string;
begin
 s:='select max(cast(fmh_nomor AS DECIMAL)) from tfeemarketing_hdr ' ;
...
procedure TfrmSetingFeeMarketing.cxButton1Click(Sender: TObject);
begin
    try
...
procedure TfrmSetingFeeMarketing.cxButton8Click(Sender: TObject);
begin
Release;
end;

procedure TfrmSetingFeeMarketing.cxButton2Click(Sender: TObject);
begin
   try
...
procedure TfrmSetingFeeMarketing.FormCreate(Sender: TObject);
begin
...
function TfrmSetingFeeMarketing.GetCDS: TClientDataSet;
begin
...
function TfrmSetingFeeMarketing.GetCDScustomer: TClientDataset;
var s:String;
begin
...
procedure TfrmSetingFeeMarketing.FormShow(Sender: TObject);
begin
refreshdata;
end;

procedure TfrmSetingFeeMarketing.clNoGetDisplayText(Sender: TcxCustomGridTableItem;
...
procedure TfrmSetingFeeMarketing.HapusRecord1Click(Sender: TObject);
begin
...
procedure TfrmSetingFeeMarketing.simpandata;
var
  s:string;
  atax,i:integer;
  tt:TStrings;
...
if FLAGEDIT then
  s:='update tfeemarketing_hdr set '
    + ' fmh_cus_kode = ' + Quot(cxLookupcustomer.EditValue)
    + ' where fmh_nomor = ' + quot(FID) + ';'
else
begin
  edtNomor.Text := getmaxkode;
  s :=  ' insert into tfeemarketing_hdr '
             + ' (fmh_nomor, fmh_cus_kode'
             + ' ) '
             + ' values ( '
             + Quot(edtNomor.Text) + ','
   ...
   s:= ' delete from tfeemarketing_dtl '
     + ' where fmd_fmh_nomor =' + quot(FID);
  ...
  begin
    S:='insert into tfeemarketing_dtl (fmd_fmh_nomor,fmd_brg_kode,fmd_persen,fmd_rupiah) values ('
      + Quot(edtNomor.Text) +','
...
function TfrmSetingFeeMarketing.cekdata:Boolean;
var
  i:integer;
...
procedure TfrmSetingFeeMarketing.loaddataall(akode : string);
var
  s: string ;
  tsql : TmyQuery;
  a,i:Integer;
...
begin
  ...
  s := ' SELECT fmh_nomor,fmh_cus_kode,cus_nama,cus_alamat, '
+ ' fmd_brg_kode kode,brg_nama nama,fmd_persen persen,fmd_rupiah rupiah '
+ ' FROM tfeemarketing_hdr '
+ ' INNER JOIN tfeemarketing_dtl a ON fmh_nomor=fmd_fmh_nomor '
+ ' inner join tcustomer on cus_kode=fmh_cus_kode '
+ ' inner join tbarang on brg_kode=fmd_brg_kode '
+ ' WHERE fmh_nomor = '+ Quot(akode);
   ...
   try
       ...
       begin
         ...
         begin
            flagedit := True;
            FID :=fieldbyname('fmh_nomor').AsString;
            edtnomor.Text := fieldbyname('fmh_nomor').AsString;
            cxLookupcustomer.EditValue  := fieldbyname('fmh_cus_kode').AsString;
...
procedure TfrmSetingFeeMarketing.cxLookupcustomerPropertiesEditValueChanged(
  Sender: TObject);
begin
...
function TfrmSetingFeeMarketing.GetCDSRekeningCash: TClientDataset;
var s:String;
begin
...
procedure TfrmSetingFeeMarketing.clStatusPropertiesEditValueChanged(
  Sender: TObject);
begin
   CDS.Post;
   If CDS.State <> dsEdit then CDS.Edit;
...
procedure TfrmSetingFeeMarketing.clBayarPropertiesEditValueChanged(
  Sender: TObject);
begin
 CDS.Post;
...
procedure TfrmSetingFeeMarketing.BitBtn1Click(Sender: TObject);
var
  awal,akhir :string;
  i:Integer;
begin
...
procedure TfrmSetingFeeMarketing.clKodePropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
 bantuansku;
end;

procedure TfrmSetingFeeMarketing.bantuansku;
  var
    s:string;
    tsql2,tsql:TmyQuery;
    i:Integer;
...
procedure TfrmSetingFeeMarketing.BitBtn2Click(Sender: TObject);
var
  s:string;
  tsql: TmyQuery;
begin
